...
Locate the actual confirmation-screen component(s) — likely app/register/complete/page.tsx and app/register/founder/complete/page.tsx (or a client component either renders) — confirm real paths before editing, do not assume from this DIP's description alone.
In each screen where the gap applies, read role from the authenticated registrant's session/JWT app_metadata after successful registration (same mechanism FP-102 already established — reuse it, don't re-derive a new way to read the claim).
...
The app-download message/link may remain a placeholder (mobile app doesn't exist yet) — no change needed there.
No change to complete_registration() or any other part of the registration-completion transaction (members/assignments/invitations writes) — confirm this DIP doesn't touch that function at all.
