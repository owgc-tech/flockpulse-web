...
// GET /api/tasks/auto-assign/food-assignment/slots?event_type_ids=a,b,c —
// every open Food Assignment slot on an upcoming (DRAFT/SCHEDULED/ACTIVE)
// event whose type is in the given comma-separated list, for the
// auto-assign panel's slot list and roster-scoped summary table. A
// missing/empty param becomes [], which returns an empty slot list rather
// than an error — every event type starts unchecked on the panel, so this
// is the actual default state (DIP-FP-180-adj-4), not a failure mode.
export async function GET(req: NextRequest) {
  return withAuth(req, requireLeader(async (_, ctx) => {
    ...
    try {
      const { slots } = await getFoodAssignmentAutoAssignData(ctx.tenantId, eventTypeIds);
