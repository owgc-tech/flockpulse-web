...
// (individuals only) across every open Prayer Leader slot on an upcoming
// event whose type is in event_type_ids. Unconditionally overwrites existing
// assignees — the confirm-before-run warning lives client-side, not here.
export async function POST(req: NextRequest) {
  return withAuth(req, requireLeader(async (_, ctx) => {
    ...
    try {
      const assignments = await runPrayerLeaderAutoAssign(ctx.tenantId, roster, ctx.memberId, event_type_ids);
