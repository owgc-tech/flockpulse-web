...
import { getRsvpReport } from '@/src/features/reports/report.service';

// GET /api/reports/rsvp?event_id=&group_id=&member_id=
//
// RBAC (mirrors Confirmations' access level, per STORY-9.1):
...
export async function GET(req: NextRequest) {
  return withAuth(req, requireRole('LEADER')(async (_, ctx) => {
    ...
    try {
      const data = await getRsvpReport(ctx.tenantId, ctx.memberId, ctx.role, { eventId, groupId, memberId });
